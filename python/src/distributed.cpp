...
void init_distributed(nb::module_& parent_module) {
  ...
  m.def("all_to_all",
    [](const ScalarOrArray& x, std::optional<mx::distributed::Group> group,
       mx::StreamOrDevice s) {
      return mx::distributed::all_to_all(to_array(x), group, s);
    },
    "x"_a, nb::kw_only(), "group"_a = nb::none(), "stream"_a = nb::none(),
    nb::sig("def all_to_all(x: array, *, group: Optional[Group] = None, "
  ...
  m.def(
      ...
      [](std::optional<mx::distributed::Group> group,
         ...
         int hidden_dim,
         mx::Dtype dtype) {
        mx::distributed::moe_ep_warmup(group, num_experts, capacity, hidden_dim, dtype);
      },
      nb::kw_only(),
      "group"_a = nb::none(),
      "num_experts"_a = 0,
