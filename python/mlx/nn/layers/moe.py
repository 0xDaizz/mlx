...
class MixtureOfExperts(Module):
    ...
    def _get_stacked_weights(self):
        ...
        if not hasattr(self, "_cached_stacked") or self._cached_stacked is None:
            ...
            self._cached_stacked = (
                mx.stack([e.w_gate.weight for e in self.experts]).swapaxes(
                    -1, -2
                ),  # [E, D, expert_dim]
                mx.stack([e.w_up.weight for e in self.experts]).swapaxes(
                    -1, -2
                ),  # [E, D, expert_dim]
                mx.stack([e.w_down.weight for e in self.experts]).swapaxes(
                    -1, -2
                ),  # [E, expert_dim, D]
            )
    ...
    def _run_local_experts_batched_chunked(
        self, dispatched: mx.array, chunk_e: int
    ) -> mx.array:
