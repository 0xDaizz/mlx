...
MoeBackend MoePolicy::resolve(int N, int top_k, int D, int elem_size) {
  ...
  int64_t gpu_switch_bytes =
      env_int("MLX_MOE_EP_GPU_SWITCH_BYTES", 2 * 1024 * 1024);
  int64_t work_bytes = static_cast<int64_t>(N) * top_k * D * elem_size;
...
void MoePolicy::record_metal_failure() {
  ...
  cooldown_until_ms.store(now_ms() + cooldown_ms, std::memory_order_relaxed);
