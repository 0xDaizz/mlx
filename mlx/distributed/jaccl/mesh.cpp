...
void MeshGroup::blocking_all_to_all(const array& input, array& output) {
  ...
  while (in_flight > 0) {
    ...
    if (n < 0) {
      throw std::runtime_error(
          "[jaccl] blocking_all_to_all: poll() returned " +
          std::to_string(n));
    }
      ...
      if (work_type == SEND_WR) {
        if (read_offset < per_peer_bytes) {
          std::copy(
              ...
              send_src +
                  std::min(
                      read_offset + static_cast<int64_t>(N),
                      per_peer_bytes),
              send_buffer(sz, b).begin<char>());
          send_to(sz, peer, b);
          in_flight++;
...
void MeshGroup::blocking_sendrecv(
    ...
    int peer, detail::ExchangeTag tag) {
  // Skip no-op
  ...
  // But since existing code uses wr_id as: work_type(16-23) | buff(8-15) | rank(0-7)
  // and poll() only looks at bits 0-23, we can safely put tag in bits 48-63.
  // However, ibv_wc.wr_id is uint64_t, so this is safe.
  //
  // Actually, to keep it simple and avoid any potential issues with existing code,
  // we'll just use the standard wr_id format (SEND_WR/RECV_WR, buff, rank) and
  // NOT encode the tag in wr_id. The tag is implicit since blocking_sendrecv
  // uses its own poll loop and won't see other CQ entries from other operations
  // (we're in a blocking context inside enc.dispatch lambda).
  ...
  if (max_bytes == 0) max_bytes = 1; // avoid 0-size
  ...
  while (in_flight > 0) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (work_type == SEND_WR) {
        ...
        auto chunk = std::min(
            static_cast<int64_t>(N), recv_total - recv_write_offset);
        if (chunk > 0) {
          std::copy(
              recv_buffer(sz, b, peer).begin<char>(),
        ...
        if (recv_write_offset + (PIPELINE - 1) * static_cast<int64_t>(N) < recv_total) {
          recv_from(sz, peer, b);
          in_flight++;
        }
      }
...
void MeshGroup::all_to_all(const array& input, array& output, Stream stream) {
  ...
  encoder.dispatch([in_ptr, out_ptr, n_bytes, this]() {
    ...
    while (read_offset < per_peer_bytes && buff < PIPELINE) {
      ...
      std::copy(
          send_src + read_offset,
          send_src + std::min(read_offset + static_cast<int64_t>(N), per_peer_bytes),
          send_buffer(sz, buff).begin<char>());
      send_to(sz, peer, buff);
      in_flight++;
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (work_type == SEND_WR) {
          ...
          std::copy(
              recv_buffer(sz, b, peer).begin<char>(),
              recv_buffer(sz, b, peer).begin<char>() +
                  std::min(static_cast<int64_t>(N), per_peer_bytes - write_offset),
              recv_dst + write_offset);
          write_offset += N;
