...
struct MPIWrapper {
  ...
  MPI_Datatype mpi_byte() { return mpi_byte_; }
  ...
  int (*all_to_all)(const void*, int, MPI_Datatype, void*, int, MPI_Datatype, MPI_Comm);
  int (*sendrecv)(const void*, int, MPI_Datatype, int, int,
...
class MPIGroup : public GroupImpl {
 public:
  ...
  void all_to_all(const array& input, array& output, Stream stream) override {
    ...
    encoder.dispatch(mpi().all_to_all,
        input.data<void>(), count, mpi().datatype(input),
  ...
  void blocking_sendrecv(
      ...
      int peer, detail::ExchangeTag tag) override {
    ...
    while (so < send_nbytes || ro < recv_nbytes) {
      int sc = (so < send_nbytes)
          ? static_cast<int>(std::min(static_cast<size_t>(INT_MAX), send_nbytes - so))
          : 0;
      int rc = (ro < recv_nbytes)
          ? static_cast<int>(std::min(static_cast<size_t>(INT_MAX), recv_nbytes - ro))
          : 0;

      MPI_Status st;
      mpi().sendrecv(
          sc > 0 ? sptr + so : sptr, sc, mpi().mpi_byte(), peer, mpi_tag,
          rc > 0 ? rptr + ro : rptr, rc, mpi().mpi_byte(), peer, mpi_tag,
          comm_, &st);
