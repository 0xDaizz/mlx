...
// Zero-copy variant that reads from two separate source buffers (local expert_out
// and remote response buffer) instead of requiring a unified memcpy'd buffer.
// src_which[nk] = 0 for local, 1 for remote, -1 to skip.
//
// Grid: (D_ceil, N, 1)
// Group: (min(D_ceil, 256), 1, 1)
template <typename T>
[[kernel]] void moe_combine_weighted_sum_dual_src(
    const device T* local_src   [[buffer(0)]],   // expert_out [E_local * cap_total * D]
    ...
    const device int* src_idx   [[buffer(5)]],   // [N * top_k] row index into respective buffer
    const device int* src_which [[buffer(6)]],   // [N * top_k] 0=local, 1=remote, -1=skip
