...
void MoeDispatchExchange::eval_gpu(
  ...
  if (world_size > 2) {
    MoeDispatchExchange cpu_fb(
        ...
        group(), num_experts_, capacity_, deterministic_, MoeBackend::Cpu);
  ...
  if (world_size == 1) {
    ...
    array slot_map_buf(
        {valid_count}, int32, nullptr, {});
    slot_map_buf.set_data(
        allocator::malloc(valid_count * sizeof(int32_t)));
    std::memcpy(
        slot_map_buf.data<int32_t>(),
        slot_map_vec.data(),
        valid_count * sizeof(int32_t));

    array nk_indices_buf(
        {valid_count}, int32, nullptr, {});
    nk_indices_buf.set_data(
        allocator::malloc(valid_count * sizeof(int32_t)));
    std::memcpy(
  ...
  // =========================================================================
  {
    ...
    if (local_count > 0) {
      ...
      slot_map_buf.set_data(
          allocator::malloc(local_count * sizeof(int32_t)));
      std::memcpy(
          slot_map_buf.data<int32_t>(),
          local_slot_map.data(),
    ...
    array send_pkt(
        {static_cast<int>(send_pkt_bytes)}, uint8, nullptr, {});
    send_pkt.set_data(allocator::malloc(send_pkt_bytes));

    array recv_pkt(
        {static_cast<int>(recv_pkt_bytes)}, uint8, nullptr, {});
    ...
    if (send_count > 0) {
      ...
      src_idx_buf.set_data(
          allocator::malloc(send_count * sizeof(int32_t)));
      std::memcpy(
      ...
      headers_buf.set_data(
          allocator::malloc(send_count * sizeof(uint32_t)));
      std::memcpy(
    ...
    if (peer_count > 0) {
      ...
      for (int i = 0; i < peer_count; i++) {
        const uint8_t* row = recv_pkt_ptr +
      ...
      flat_idx_buf.set_data(
          allocator::malloc(peer_count * sizeof(int32_t)));
      std::memcpy(
    ...
    MoeMetrics::global().fallback_to_cpu_count.fetch_add(1, std::memory_order_relaxed);

    // Flush any partial GPU state before CPU fallback
    try { gpu::synchronize(stream()); } catch (...) {}
...
void MoeCombineExchange::eval_gpu(
  ...
  // =========================================================================
  {
    ...
    // For ws==2, unified_src = expert_out (possibly copied) + scattered responses

    // We track:
    // - local_entries: (nk_idx, flat_idx_in_expert_out) for local accumulation
    ...
    array req_send(
        {static_cast<int>(req_send_bytes)}, uint8, nullptr, {});
    req_send.set_data(allocator::malloc(req_send_bytes));
    array req_recv(
        {static_cast<int>(req_recv_bytes)}, uint8, nullptr, {});
    ...
    for (int i = 0; i < peer_req_count; i++) {
      ...
      // expert_out flat index: expert_out[local_expert, peer*capacity+slot_pos]
    ...
    array resp_send(
        {static_cast<int>(resp_send_bytes)}, uint8, nullptr, {});
    resp_send.set_data(allocator::malloc(resp_send_bytes));
    array resp_recv(
        {static_cast<int>(resp_recv_bytes)}, uint8, nullptr, {});
    ...
    if (use_zero_copy) {
      ...
      // that reads from expert_out (local) and remote_buf (remote) separately.
      ...
      if (peer_res_count > 0) {
        ...
        array remote_buf(
            {peer_res_count, D_val}, dtype, nullptr, {});
        remote_buf.set_data(allocator::malloc(
        ...
        auto ws_kernel = get_moe_kernel(d, "moe_combine_weighted_sum_dual_src", dtype);
        ...
        // No remote data — all local. Use dual-src kernel with empty remote_buf.
        // Allocate minimal remote_buf (1 element to avoid null pointer)
      ...
      array unified_src(
          {unified_total_rows, D_val}, dtype, nullptr, {});
