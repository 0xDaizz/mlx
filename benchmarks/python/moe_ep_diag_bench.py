...
def run_overhead_isolation(group, E, D, top_k, capacity, dtype,
                           warmup, iters, rank):
    ...
    for N in N_VALUES:
        ...
        for backend in ["cpu", "auto"]:
            ...
            try:
                fn = make_dispatch_combine_fn(
                    tokens, expert_indices, weights_f32,
                    E, capacity, group, backend,
                )
        ...
        if (not np.isnan(cpu_med) and not np.isnan(auto_med)
                and cpu_med > 0):
        ...
        rows_combined.append([
            N,
            cpu_med, results["cpu"][1],
            auto_med, results["auto"][1],
            delta_ms, delta_pct,
        ])
...
def run_crossover_sweep(group, E, D, top_k, capacity, dtype,
    ...
    for N in N_VALUES:
        ...
        for backend in ["cpu", "metal"]:
            barrier(group)
            try:
                fn_d = make_dispatch_fn(tokens, expert_indices, E, capacity, group, backend)
        ...
        if (not np.isnan(cpu_med) and not np.isnan(metal_med)
                and metal_med > 0):
        ...
        rows.append([
            N,
            cpu_med, metal_med, ratio, winner,
            cpu_d, cpu_c, mtl_d, mtl_c,
        ])
    ...
    if rank == 0:
        ...
        if crossover_n is not None:
            ...
            print(f"      dispatch_buf = {dispatch_buf_bytes / 1024 / 1024:.1f} MB, "
...
def run_routing_analysis(group, E, D, top_k, capacity, dtype,
                         warmup, iters, rank):
    ...
    if rank == 0:
        ...
        print(f"    E={E}, E_local={E_local}, D={D}, top_k={top_k}, capacity={capacity}")
    ...
    for N in N_VALUES:
        ...
        try:
            ...
            if valid_count > 0:
                ...
                remote_mask = (valid_experts < local_start) | (valid_experts >= local_end)
            ...
            utilization = valid_count / total_buffer_slots * 100.0 if total_buffer_slots > 0 else 0.0

            # Also time dispatch+combine for latency context
            barrier(group)
            fn = make_dispatch_combine_fn(
                tokens, expert_indices, weights_f32,
                E, capacity, group, "cpu",
            )
            ...
            fn = make_dispatch_combine_fn(
                tokens, expert_indices, weights_f32,
                E, capacity, group, "metal",
            )
            med_metal, _, _ = time_fn(fn, warmup, iters)

            rows.append([
                N, total_slots, overflow_count, overflow_ratio,
                valid_count, remote_count, remote_ratio,
                utilization, med_cpu, med_metal,
            ])
    ...
    if rank == 0:
        ...
        for N in N_VALUES:
            expected = N * top_k / E
            print(f"      N={N:>5d}: expected {expected:.2f} tokens/expert "
...
def main():
    ...
    if rank == 0:
        ...
        print("=" * 100)
        print(
            f"Diagnostic Benchmark — MoE EP Overhead, Crossover & Routing Analysis"
        )
        print(
            f"  E={E}, top_k={top_k}, D={D}, capacity={capacity}, dtype=float16"
        )
        print(
    ...
    run_overhead_isolation(group, E, D, top_k, capacity, dtype,
                           warmup, iters, rank)

    # ── Q2: Crossover sweep ───────────────────────────────────────────────
    run_crossover_sweep(group, E, D, top_k, capacity, dtype,
                        warmup, iters, rank)

    # ── Q3: Routing analysis ──────────────────────────────────────────────
    run_routing_analysis(group, E, D, top_k, capacity, dtype,
                         warmup, iters, rank)
    ...
    if rank == 0:
        ...
        print("  delta_ms   = auto_time - cpu_time (overhead of auto backend selection)")
        print("  delta_%    = (auto / cpu - 1) * 100% (relative overhead)")
        print("  oflow%     = overflow ratio: tokens hitting capacity limit / (N * top_k)")
        print("  remote%    = remote ratio: tokens sent to non-local experts / valid tokens")
