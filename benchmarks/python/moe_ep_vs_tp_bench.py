...
def main():
    ...
    parser.add_argument(
        "--E", type=int, default=384, help="Total number of experts (default 384)"
    )
    parser.add_argument(
        "--D", type=int, default=7168, help="Hidden dimension (default 7168)"
    )
    parser.add_argument(
        "--top-k", type=int, default=8, help="Top-k experts per token (default 8)"
    )
    parser.add_argument(
        "--intermediate",
        type=int,
    ...
    parser.add_argument(
        "--iters", type=int, default=30, help="Timing iterations (default 30)"
    )
    ...
    if rank == 0:
        ...
        print(
            f"  E={E}  D={D}  top_k={top_k}  intermediate={intermediate}" f"  cf={cf}"
        )
        print(f"  C++ fused EP path: {'available' if has_cpp else 'NOT available'}")
        print(sep)
    ...
    for N in N_VALUES:
        ...
        if rank == 0:
            print(
                f"  N={N:>5d}  cap={capacity:>4d}  N_ep={N_ep:>5d} ...",
                end="",
                flush=True,
            )
    ...
    if rank == 0:
        ...
        # ── Geomean summaries ──────────────────────────────────────────
        all_ratios = [r[4] for r in suite2_rows if not np.isnan(r[4]) and r[4] > 0]
        decode_ratios = [
            r[4] for r in suite2_rows if r[0] <= 64 and not np.isnan(r[4]) and r[4] > 0
        ]
        prefill_ratios = [
            r[4] for r in suite2_rows if r[0] >= 256 and not np.isnan(r[4]) and r[4] > 0
        ]
        ...
        if not np.isnan(gm_all):
            if gm_all <= 1.0:
                judgment = (
                    f"EP wins vs TP — geomean EP/TP = {gm_all:.2f} (<= 1.0 = EP wins)"
                )
            else:
                judgment = (
                    f"EP loses vs TP — geomean EP/TP = {gm_all:.2f} (> 1.0 = TP wins)"
                )
        else:
        ...
        print(
            f"  swiglu_shrd  = SwiGLU FFN with sharded weights"
            f" (D={D}, inter={intermediate // ws})"
        )
        print(
            f"  ep_d+c       = moe_dispatch_exchange + moe_combine_exchange"
            f" (backend=auto, cf={cf})"
        )
        print(
            f"  swiglu_ful   = SwiGLU FFN with full weights"
